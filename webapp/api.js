...
const updateRecipe = async (req, res) => {
	...
	const nutritionSchema = Joi.object().keys({
		...
		cholestrol_in_mg: Joi.number().optional(),
		sodium_in_mg: Joi.number().integer().optional(),
		carbohydrates_in_grams: Joi.number().optional(),
		protein_in_grams: Joi.number().optional(),
	}).optional();
    ...
    const newSteps = req.body.steps ? req.body.steps : recipeSteps;
   ...
   await db.updateRecipe(updateRecipeInput).then(() => {
        res.status(204).send();
	});
	...
	const newNutrition_information = req.body.nutrition_information ? req.body.nutrition_information : recipeNutritionInformaiton;
	await db.updateRecipeNutritionInformation(newNutrition_information);
}
